Capistrano::Configuration.instance(:must_exist).load do
  ...
  namespace :postgresql do
    ...
    task :create_database, :roles => :db, :only => { :primary => true } do
      run %Q{#{sudo} -h #{postgresql_host} -u postgres psql -c "create user #{postgresql_user} with password '#{postgresql_password}';"}
      run %Q{#{sudo} -h #{postgresql_host} -u postgres psql -c "create database #{postgresql_database} owner #{postgresql_user};"}
      run %Q{#{sudo} -h #{postgresql_host} -u postgres psql -d #{postgresql_database} -c "CREATE EXTENSION hstore;"}
    end
